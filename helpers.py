...
def get_system_identity(snapshot):
    info = {}
    try:
        # Base registry path for the Windows version information.
        base_key = r"SOFTWARE\Microsoft\Windows NT\CurrentVersion"

        # Read the friendly operating system name like "Windows 11 Enterprise".
        info["os_name"] = get_registry_value(
            winreg.HKEY_LOCAL_MACHINE,
            base_key,
            "ProductName"
        )  # This is a string value in the registry.

        # Read the build number such as "22631".
        info["os_build"] = get_registry_value(
            winreg.HKEY_LOCAL_MACHINE,
            base_key,
            "CurrentBuild"
        )  # Also stored as a string in the registry.

        # Read the edition identifier like "Enterprise" or "Professional".
        ...
        # Read the registered owner string entered during installation.
        ...
        # Read the raw install timestamp from the registry.
        install_epoch = get_registry_value(
            winreg.HKEY_LOCAL_MACHINE,
            base_key,
            "InstallDate"
        )  # This is a Unix epoch integer, not a human-readable date.

        if install_epoch is None:
            info["install_date_utc"] = None
        else:
            try:
                # Convert the epoch to a UTC ISO string, matching the spec.
                # I chose isoformat() + "Z" because it is standard and easy
                # to compare across systems. Another valid choice would be
                # str(datetime.datetime.utcfromtimestamp(...)), but that is
                # less standardized for JSON output.
                info["install_date_utc"] = (
                    datetime.datetime.utcfromtimestamp(int(install_epoch))
                    .isoformat() + "Z"
                )
            except Exception:
                # If the registry value is malformed, keep the field None
                # instead of crashing the whole snapshot collection.
                info["install_date_utc"] = None
    except Exception as e:
...
def get_installed_software(snapshot):
    ...
    # Helper converts an YYYYMMDD string into ISO date format.
    # I chose datetime.strptime() because it validates the format.
    # A simpler option would be raw slicing, but that would not catch
    # invalid date strings as cleanly.
    def normalize_install_date(raw_date):
        ...
        if cleaned == "":
            return None
        try:
            return datetime.datetime.strptime(cleaned, "%Y%m%d").date().isoformat()
        except Exception:
            return None

    try:
        # Open the parent Uninstall key so we can enumerate its children.
        ...
        while True:
            try:
                # winreg.EnumKey returns the subkey name at the given index.
                # It raises OSError when there are no more subkeys, so the
                # loop ends by catching that specific exception.
            ...
            # Read DisplayName first; skip entries without it.
